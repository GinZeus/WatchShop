    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (account == null) {
            ...
            if (account.getRole_id() != 2) { // check if acc is manager
                ...
                Brand brand = new Brand(Integer.parseInt(brand_id),brand_name,brand_address,brand_status);
                dao.editBrandInfo(brand);
                
                ProductDAO productDAO = new ProductDAO();
                productDAO.updateStatusOfProductByBrand(brand_id, brand_status); // change status of product based on brand status
