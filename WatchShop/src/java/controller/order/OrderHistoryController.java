...
public class OrderHistoryController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (acc != null) {
            if (acc.getRole_id() == 4) {
                ArrayList<Order> listO = od.getOrdersByStatus(Integer.toString(acc.getAcc_id()), 5);
                request.setAttribute("listO", listO);

                request.getRequestDispatcher("Order_history.jsp").forward(request, response);
            } 
            else if (acc.getRole_id() == 3){
                ArrayList<Order> listO = od.getListOrders(5);
                request.setAttribute("listO", listO);
                request.getRequestDispatcher("Manage_History.jsp").forward(request, response);
            }
            else {
