...
public class ChangeOrderStatus extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc != null) {
            if (acc.getRole_id() == 3)  {
                if (od.getOrderByOId(o_id).getStatus() == 1) {
                    od.updateOrderStatus(o_id, status);

                    response.sendRedirect("ToConfirmController");
                } else if (od.getOrderByOId(o_id).getStatus() == 2) {
                    od.updateOrderStatus(o_id, status);

                    response.sendRedirect("ToPackController");
                }
            } else {
